...
class Medias extends React.Component {
  constructor(props) {
    ...
    this.handleClickOnPdf = this.handleClickOnPdf.bind(this);
  }

  handleClickOnPdf(doc) {
    const {thread, threadType, history} = this.props;
    if (doc && doc.index >= MAX_GRID_IMAGES - 1 && threadType && thread) {
  ...
  render() {
    const {medias, GridImageProps, imagesAdornment, videosAdornment, documentsAdornment, linksAdornment} = this.props;
    if (!medias.length) {
      ...
      <Root>
        ...
        {links.length > 0 && (
          <div className={classNames(classes.medias, classes.links)}>
            {linksAdornment}
            {links.map((p, i) => (
              <Link key={i} media={p} wide={medias.length === 1} />
            ))}
          </div>
        )}
      </Root>
...
Medias.propTypes = {
  ...
  GridImageProps: PropTypes.object,
  imagesAdornment: PropTypes.node,
  videosAdornment: PropTypes.node,
  documentsAdornment: PropTypes.node,
  linksAdornment: PropTypes.node,

  /* Translation */
  intl: PropTypes.object.isRequired
};

export default injectIntl(Medias);
