import React, {forwardRef, ForwardRefRenderFunction, FunctionComponent, useEffect, useImperativeHandle, useRef, useState} from 'react';
...
export type TRichTextEditorRef = {
  focus: () => void;
  save: () => void;
...
type IRichTextEditorProps = TRichTextEditorProps;

type TRichTextEditorState = {
...
const RichTextEditor: ForwardRefRenderFunction<TRichTextEditorRef, IRichTextEditorProps> = (props, ref) => {
  const {controls, customControls} = props;

  const [state, setState] = useState<TRichTextEditorState>({});
  ...
  const handleInsertAtomicBlockAsync = (name: string, promise: Promise<TAsyncAtomicBlockResponse>, placeholder?: string) => {
    ...
    promise
      ...
      .catch((error) => {
        ...
        const newContentState = Modifier.removeRange(editorStateRef.current.getCurrentContent(), newSelection, 'forward');
