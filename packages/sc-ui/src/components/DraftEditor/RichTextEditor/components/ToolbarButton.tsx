...
const ToolbarButton: FunctionComponent<ToolbarButtonProps> = (props) => {
  const size = !props.inlineMode ? props.size || 'medium' : 'small';
  const toolbarId = props.inlineMode ? '-toolbar' : '';
  const editorId = props.editorId || 'rte';
  ...
  if (props.icon) {
    return (
      <IconButton {...sharedProps} aria-label={props.label} color={props.active ? 'primary' : 'default'} size={size}>
        {props.icon}
      </IconButton>
    );
  }
