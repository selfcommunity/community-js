import {EditorState, DraftBlockType, ContentBlock, ContentState, Modifier, SelectionState, getVisibleSelectionRect, DraftStyleMap} from 'draft-js';
...
const atomicBlockExists = (name: string, controls?: TCustomControl[]) => {
  ...
  return controls.find((control) => control.type === 'atomic' && control.name === name && control.atomicComponent !== undefined);
};
...
const clearInlineStyles = (editorState: EditorState, customStyles?: DraftStyleMap): ContentState => {
  ...
  return styles.reduce(
    (newContentState: ContentState, style: string) => Modifier.removeInlineStyle(newContentState, editorState.getSelection(), style),
