...
import {useKrispNoiseFilter} from '@livekit/components-react/src/hooks/cloud/krisp/useKrispNoiseFilter';
 */
/**
 * @alpha
 */
    ...
    <div className="settings-menu" style={{width: '100%'}} {...props}>
      <div>
        {tabs.map(
          (tab) =>
            settings[tab] && (
              <button key={tab} onClick={() => setActiveTab(tab)} aria-pressed={tab === activeTab}>
                {
          ...
          <>
            ...
            <section>
              ...
              <input
                ...
                disabled={isNoiseFilterPending}></input>
            </section>
          </>
        )}
        {activeTab === 'recording' && (
      ...
      <button onClick={() => layoutContext?.widget.dispatch?.({msg: 'toggle_settings'})}>Close</button>
    </div>
  );
} **/
