...
import TrendingFeedWidgetSkeleton from '../TrendingFeedWidget/Skeleton';
...
export interface RelatedFeedObjectsWidgetProps extends VirtualScrollerItemProps {
  /**
 ...
 *> API documentation for the Community-JS Related FeedObjects component. Learn about the available props and the CSS API.
 ...
 |no-results|.SCRelatedFeedObjectsWidget-no-results|Styles applied to no results section.|
 |relatedItem|.SCRelatedFeedObjectsWidget-related-item|Styles applied to the related item element.|
 |showMore|.SCRelatedFeedObjectsWidget-show-more|Styles applied to show more button element.|

 *
 * @param inProps
 */
export default function RelatedFeedObjectsWidget(inProps: RelatedFeedObjectsWidgetProps): JSX.Element {
  // CONST
  const limit = 4;
  ...
  // PROPS
  const props: RelatedFeedObjectsWidgetProps = useThemeProps({
    props: inProps,
    name: PREFIX
  });
  const {
  ...
  if (state.isLoadingNext) {
    return <TrendingFeedWidgetSkeleton />;
  }
