...
const PREFIX = 'SCTrendingFeedWidget';
...
export interface TrendingFeedWidgetProps extends VirtualScrollerItemProps {
  /**
 ...
 * > API documentation for the Community-JS Trending Feed Widget component. Learn about the available props and the CSS API.
 ...
 ```jsx
 import {TrendingFeedWidget} from '@selfcommunity/react-ui';
 ```

 #### Component Name

 The name `SCTrendingFeedWidget` can be used when providing style overrides in the theme.
 ...
 |---|---|---|
 |root|.SCTrendingFeedWidget-root|Styles applied to the root element.|
 |title|.SCTrendingFeedWidget-title|Styles applied to the title element.|
 |noResults|.SCTrendingFeedWidget-no-results|Styles applied to no results section.|
 |trendingItem|.SCTrendingFeedWidget-trending-item|Styles applied to the trending feed item element.|
 |showMore|.SCTrendingFeedWidget-show-more|Styles applied to show more button element.|

 * @param inProps
 */
export default function TrendingFeedWidget(inProps: TrendingFeedWidgetProps): JSX.Element {
  //CONST
  const limit = 4;

  // PROPS
  const props: TrendingFeedWidgetProps = useThemeProps({
    props: inProps,
    name: PREFIX
  });
