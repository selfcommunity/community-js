...
const PREFIX = 'SCTrendingPeopleWidget';
...
export interface TrendingPeopleWidgetProps extends VirtualScrollerItemProps {
  /**
   * Category id
   * @default null
   */
 ...
 * > API documentation for the Community-JS Trending People Widget component. Learn about the available props and the CSS API.
 ...
 ```jsx
 import {TrendingPeopleWidget} from '@selfcommunity/react-ui';
 ```

 #### Component Name

 The name `SCTrendingPeopleWidget` can be used when providing style overrides in the theme.
 ...
 |---|---|---|
 |root|.SCTrendingPeopleWidget-root|Styles applied to the root element.|
 |title|.SCTrendingPeopleWidget-title|Styles applied to the title element.|
 |noResults|.SCTrendingPeopleWidget-no-results|Styles applied to no results section.|
 |trendingUserItem|.SCTrendingPeopleWidget-trending-user-item|Styles applied to the trending user item element.|
 |showMore|.SCTrendingPeopleWidget-show-more|Styles applied to show more button element.|

 * @param inProps
 */
export default function TrendingPeopleWidget(inProps: TrendingPeopleWidgetProps): JSX.Element {
  // CONST
  const limit = 3;

  // PROPS
  const props: TrendingPeopleWidgetProps = useThemeProps({
    props: inProps,
    name: PREFIX
  });
  const {
